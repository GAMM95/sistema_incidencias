$(document).ready(function () {
  ...
  function nuevaConsulta() {
    ...
    $('#fechaInicio_incidencias_totales, #fechaFin_incidencias_totales').val('');

    // Realiza una llamada AJAX para obtener los registros para el rango de fechas
    $.ajax({
      url: "reportes.php?action=consultarTotales",
      type: 'GET',
  ...
  $('#limpiarCampos_incidencias_totales').on('click', nuevaConsulta);

  $('#formConsultarIncidenciasTotales').submit(function (event) {
    ...
    function validarCampos() {
      ...
      var fechaInicioSeleccionada = ($('#fechaInicio_incidencias_totales').val() !== null && $('#fechaInicio_incidencias_totales').val().trim() !== '');
      var fechaFinSeleccionada = ($('#fechaFin_incidencias_totales').val() !== null && $('#fechaFin_incidencias_totales').val().trim() !== '');
  ...
  function validarFechas() {
    ...
    const fechaInicio = new Date($('#fechaInicio_incidencias_totales').val());
    const fechaFin = new Date($('#fechaFin_incidencias_totales').val());
  ...
  $('#fechaInicio_incidencias_totales, #fechaFin_incidencias_totales').on('change', function () {
    validarFechas();
  });
...
const fechaInicio = document.getElementById("fechaInicio_incidencias_totales");
const fechaFin = document.getElementById("fechaFin_incidencias_totales");
const reporteButton = document.getElementById("reporte-incidencias-fechas");
const limpiarCamposButton = document.getElementById("limpiarCampos_incidencias_totales");
