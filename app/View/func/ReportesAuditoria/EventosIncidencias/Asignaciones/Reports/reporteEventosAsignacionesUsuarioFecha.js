...
$('#reporteEventosAsignacionesUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoAsignaciones").val();
  const fechaInicio = $('#fechaInicioEventosAsignaciones').val();
  const fechaFin = $('#fechaFinEventosAsignaciones').val();
  ...
  if (!validarCamposEventosAsignacionesUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Asignaciones/getReporteAuditoriaAsignacionesUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoAsignaciones: usuario, 
      fechaInicioEventosAsignaciones: fechaInicio,
      fechaFinEventosAsignaciones: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoAsignaciones').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosAsignaciones').val();
            const fechaFinOriginal = $('#fechaFinEventosAsignaciones').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de asignaciones por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de asignaciones por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosAsignacionesUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoAsignaciones').val() !== null && $('#usuarioEventoAsignaciones').val().trim() !== '');
