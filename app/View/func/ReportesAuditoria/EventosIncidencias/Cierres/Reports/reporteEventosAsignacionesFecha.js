...
$('#reporteEventosAsignacionesFecha').click(function () {
  ...
  if (!validarCamposEventosAsignacionesFecha()) {
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Asignaciones/getReporteAuditoriaAsignacionesFecha.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de asignaciones por rango de fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de asignaciones por rango de fechas generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de asignaciones por rango de fechas generado.', 'Advertencia');
        }
      } catch (error) {
...
function validarCamposEventosAsignacionesFecha() {
  var valido = false;
  var mensajeError = '';
