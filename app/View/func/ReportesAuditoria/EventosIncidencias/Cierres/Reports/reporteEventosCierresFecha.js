...
$('#reporteEventosCierresFecha').click(function () {
  ...
  if (!validarCamposEventosCierresFecha()) {
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Cierres/getReporteAuditoriaCierresFecha.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de cierres por rango de fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de cierres por rango de fechas generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de cierres por rango de fechas generado.', 'Advertencia');
        }
      } catch (error) {
...
function validarCamposEventosCierresFecha() {
  var valido = false;
  var mensajeError = '';
