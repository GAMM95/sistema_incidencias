...
$('#reporteEventosCierresUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoCierres").val();
  const fechaInicio = $('#fechaInicioEventosCierres').val();
  const fechaFin = $('#fechaFinEventosCierres').val();
  ...
  if (!validarCamposEventosCierresUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Cierres/getReporteAuditoriaCierresUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoCierres: usuario, 
      fechaInicioEventosCierres: fechaInicio,
      fechaFinEventosCierres: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoCierres').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosCierres').val();
            const fechaFinOriginal = $('#fechaFinEventosCierres').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de cierres por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de cierres por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosCierresUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoCierres').val() !== null && $('#usuarioEventoCierres').val().trim() !== '');
