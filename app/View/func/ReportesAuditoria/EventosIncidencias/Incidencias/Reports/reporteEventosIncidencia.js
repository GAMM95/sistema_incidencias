$(document).ready(function () {
  ...
  $('#reporteEventosIncidenciasTotales').click(function () {
    // Realizar la solicitud AJAX para obtener los datos
    $.ajax({
      url: 'ajax/ReportesAuditoria/EventosIncidencias/Incidencias/getReporteAuditoriaIncidenciasTotales.php',
      method: 'GET',
      dataType: 'json',
      success: function (data) {
        ...
        try {
          ...
          doc.setProperties({
            title: 'Reporte total de eventos de incidencias.pdf',
          });
          // Mostrar mensaje de exito de pdf generado
          toastr.success('Reporte total de eventos de incidencias generado.', 'Mensaje');
          // Retrasar la apertura del PDF y limpiar el campo de entrada
          setTimeout(() => {
            window.open(doc.output('bloburl'), '_blank');
          }, 2000);
        } catch (error) {
          toastr.error('Hubo un error al generar el reporte total eventos de incidencias.', 'Mensaje de error');
          console.error('Error al generar el PDF:', error.message);
        }
      },
      error: function (xhr, status, error) {
        toastr.error('Hubo un error al obtener el reporte total de eventos de incidencias.', 'Mensaje de error');
