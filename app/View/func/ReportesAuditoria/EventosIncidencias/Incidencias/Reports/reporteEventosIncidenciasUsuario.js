...
$('#reporteEventosUsuario').click(function () {
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Incidencias/getReporteAuditoriaIncidenciasUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de incidencias por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de incidencias por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de incidencias para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de incidencias.', 'Mensaje de error');
...
function validarCamposEventosIncidenciasUsuario() {
  var valido = false;
  var mensajeError = '';
