...
$('#reporteEventosUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoIncidencias").val();
  const fechaInicio = $('#fechaInicioEventosIncidencias').val();
  const fechaFin = $('#fechaFinEventosIncidencias').val();
  ...
  if (!validarCamposEventosIncidenciasUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Incidencias/getReporteAuditoriaIncidenciasUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoIncidencias: usuario,
      fechaInicioEventosIncidencias: fechaInicio,
      fechaFinEventosIncidencias: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoIncidencias').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosIncidencias').val();
            const fechaFinOriginal = $('#fechaFinEventosIncidencias').val();
          ...
          function addTable(doc) {
            ...
            doc.autoTable({
              ...
              head: [['N°', 'FECHA Y HORA', 'USUARIO DE EVENTO', 'OPERACIÓN', 'REFERENCIA', 'ÁREA', 'IP', 'NOMBRE DEL EQUIPO']],
              ...
              columnStyles: {
                ...
                1: { cellWidth: 35 }, // Ancho para la columna fecha y hora
                2: { cellWidth: 35 }, // Ancho para la columna nombre del usuario de evento
                3: { cellWidth: 35 }, // Ancho para la columna operacion
                4: { cellWidth: 28 }, // Ancho para la columna referencia
                5: { cellWidth: 45 }, // Ancho para la columna area del usuario de evento
                6: { cellWidth: 45 }, // Ancho para la columna ip del equipo
                7: { cellWidth: 50 }, // Ancho para la columna nombre del equipo
              }
            });
          }
          ...
          doc.setProperties({
            title: 'Reporte de eventos de incidencias por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de incidencias por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosIncidenciasUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoIncidencias').val() !== null && $('#usuarioEventoIncidencias').val().trim() !== '');
