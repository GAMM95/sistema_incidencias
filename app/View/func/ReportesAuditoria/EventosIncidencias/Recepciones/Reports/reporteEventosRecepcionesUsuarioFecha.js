...
$('#reporteEventosRecepcionesUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoRecepciones").val();
  const fechaInicio = $('#fechaInicioEventosRecepciones').val();
  const fechaFin = $('#fechaFinEventosRecepciones').val();
  ...
  if (!validarCamposEventosRecepcionesUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Recepciones/getReporteAuditoriaRecepcionesUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoRecepciones: usuario,
      fechaInicioEventosRecepciones: fechaInicio,
      fechaFinEventosRecepciones: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoRecepciones').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosRecepciones').val();
            const fechaFinOriginal = $('#fechaFinEventosRecepciones').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de recepciones por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de recepciones por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosRecepcionesUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoRecepciones').val() !== null && $('#usuarioEventoRecepciones').val().trim() !== '');
