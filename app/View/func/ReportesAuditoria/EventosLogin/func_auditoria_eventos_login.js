$(document).ready(function () {
  ...
  $('#usuarioEventosLogin').change(function () {
    var selectedOption = $(this).find('option:selected');
    var codigoUsuario = selectedOption.val();
    var nombreUsuario = selectedOption.text();
    $('#codigoUsuarioEventosLogin').val(codigoUsuario);
    $('#nombreUsuarioEventosLogin').val(nombreUsuario);
  });
  ...
  $('#formAuditoriaLogin').submit(function (event) {
    ...
    if (!validarCamposEventosLogin() || !validarFechasEventosLogin()) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $('#tablaEventosLogin tbody').empty(); // Limpia el contenido actual de la tabla antes de agregar nuevos datos
        $('#tablaEventosLogin tbody').html(response); // Actualiza el contenido de la tabla con la respuesta
      },
      error: function (xhr, status, error) {
        console.error('Error en la consulta AJAX:', error);
      }
  ...
  function validarFechasEventosLogin() {
