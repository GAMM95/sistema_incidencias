$(document).ready(function () {
  ...
  $('#reporteEventosAreasTotales').click(function () {
    // Realizar la solicitud AJAX para obtener los datos
    $.ajax({
      url: 'ajax/ReportesAuditoria/EventosMantenedores/Areas/getReporteAuditoriaAreasTotales.php',
      method: 'GET',
      dataType: 'json',
      success: function (data) {
        ...
        try {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const reportTitle = 'REPORTE DE EVENTOS DE ÁREAS';
          ...
          doc.setProperties({
            title: 'Reporte total de eventos de áreas.pdf',
          });
          // Mostrar mensaje de exito de pdf generado
          toastr.success('Reporte total de eventos de &aacute;reas generado.', 'Mensaje');
          // Retrasar la apertura del PDF y limpiar el campo de entrada
          setTimeout(() => {
            window.open(doc.output('bloburl'), '_blank');
          }, 2000);
        } catch (error) {
          toastr.error('Hubo un error al generar el reporte total eventos de &aacute;reas.', 'Mensaje de error');
          console.error('Error al generar el PDF:', error.message);
        }
      },
      error: function (xhr, status, error) {
        toastr.error('Hubo un error al obtener el reporte total de eventos de &aacute;reas.', 'Mensaje de error');
