$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      var select = $('#usuarioEventoBienes');
      select.empty();
  ...
  $('#usuarioEventoBienes').change(function () {
    var selectedOption = $(this).find('option:selected');
    var codigoUsuario = selectedOption.val();
    var nombreUsuario = selectedOption.text();
    $('#codigoUsuarioEventoBienes').val(codigoUsuario);
    $('#nombreUsuarioEventoBienes').val(nombreUsuario);
  });

  // Buscador para el combo usuario
  $('#usuarioEventoBienes').select2({
    allowClear: true,
    width: '100%',
  ...
  function nuevaConsultaEventosBienes() {
    // Limpiar los campos de fecha y el input de persona (resetea el formulario)
    $('#fechaInicioEventosBienes').val('');
    $('#fechaFinEventosBienes').val('');
    $('#usuarioEventoBienes').val(null).trigger('change');  // Reset del select2 con trigger

    // Realizar la solicitud AJAX para obtener todos los registros (sin filtros)
    $.ajax({
      url: 'auditoria.php?action=consultarEventosBienes',
      type: 'GET',
      ...
      success: function (response) {
        ...
        $('#tablaEventosBienes tbody').empty();
        // Actualiza el contenido de la tabla con la respuesta
        $('#tablaEventosBienes tbody').html(response);
      },
  ...
  $('#limpiarCamposEventosBienes').on('click', nuevaConsultaEventosBienes);

  // Validación y envío del formulario
  $('#formAuditoriaBienes').submit(function (event) {
    event.preventDefault(); // Evita el envío del formulario por defecto

    // Verifica si los campos y las fechas son válidos
    if (!validarCamposEventosBienes() || !validarFechasEventosBienes()) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $('#tablaEventosBienes tbody').empty(); // Limpia el contenido actual de la tabla antes de agregar nuevos datos
        $('#tablaEventosBienes tbody').html(response); // Actualiza el contenido de la tabla con la respuesta
      },
      error: function (xhr, status, error) {
        console.error('Error en la consulta AJAX:', error);
      }
    ...
    function validarCamposEventosBienes() {
      var valido = false;
      var mensajeError = '';

      var faltaUsuario = ($('#usuarioEventoBienes').val() !== null && $('#usuarioEventoBienes').val().trim() !== '');
      var fechaInicioSeleccionada = ($('#fechaInicioEventosBienes').val() !== null && $('#fechaInicioEventosBienes').val().trim() !== '');
      var fechaFinSeleccionada = ($('#fechaFinEventosBienes').val() !== null && $('#fechaFinEventosBienes').val().trim() !== '');
  ...
  function validarFechasEventosBienes() {
    const fechaInicio = new Date($('#fechaInicioEventosBienes').val());
    const fechaFin = new Date($('#fechaFinEventosBienes').val());
  ...
  $('#fechaInicioEventosBienes, #fechaFinEventosBienes').on('change', function () { 
    validarFechasEventosBienes();
  });
});
