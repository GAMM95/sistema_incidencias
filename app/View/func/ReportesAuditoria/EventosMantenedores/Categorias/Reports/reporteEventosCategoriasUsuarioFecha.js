...
$('#reporteEventosCategoriasUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoCategorias").val();
  const fechaInicio = $('#fechaInicioEventosCategorias').val();
  const fechaFin = $('#fechaFinEventosCategorias').val();
  ...
  if (!validarCamposEventosCategoriasUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Categorias/getReporteAuditoriaCategoriasUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoCategorias: usuario,
      fechaInicioEventosCategorias: fechaInicio,
      fechaFinEventosCategorias: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            const fechaInicioOriginal = $('#fechaInicioEventosCategorias').val();
            const fechaFinOriginal = $('#fechaFinEventosCategorias').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de categorías por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de categor&iacute;as por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosCategoriasUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoCategorias').val() !== null && $('#usuarioEventoCategorias').val().trim() !== '');
