...
$('#reporteEventosPersonasUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoPersonas").val();
  const fechaInicio = $('#fechaInicioEventosPersonas').val();
  const fechaFin = $('#fechaFinEventosPersonas').val();
  ...
  if (!validarCamposEventosPersonasUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Personas/getReporteAuditoriaPersonasUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoPersonas: usuario,
      fechaInicioEventosPersonas: fechaInicio,
      fechaFinEventosPersonas: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoPersonas').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosPersonas').val();
            const fechaFinOriginal = $('#fechaFinEventosPersonas').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de personas por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de personas por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosPersonasUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoPersonas').val() !== null && $('#usuarioEventoPersonas').val().trim() !== '');
