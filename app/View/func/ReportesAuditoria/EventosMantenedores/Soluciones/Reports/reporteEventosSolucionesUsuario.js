...
$('#reporteEventosSolucionesUsuario').click(function () {
  const usuario = $("#usuarioEventoSoluciones").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosSolucionesUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Soluciones/getReporteAuditoriaSolucionesUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de soluciones por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de soluciones por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de soluciones para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de soluciones.', 'Mensaje de error');
...
function validarCamposEventosSolucionesUsuario() {
  var valido = false;
  var mensajeError = '';
