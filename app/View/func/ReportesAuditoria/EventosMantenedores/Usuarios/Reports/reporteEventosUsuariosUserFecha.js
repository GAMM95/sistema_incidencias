...
$('#reporteUserEventosUsuariosFecha').click(function () {
  const usuario = $("#usuarioEventoUsuarios").val();
  const fechaInicio = $('#fechaInicioEventosUsuarios').val();
  const fechaFin = $('#fechaFinEventosUsuarios').val();
  ...
  if (!validarCamposEventosUsuariosUserFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Usuarios/getReporteAuditoriaUsuarioUserFecha.php',
    method: 'GET',
    data: {
      usuarioEventoUsuarios: usuario,
      fechaInicioEventosUsuarios: fechaInicio,
      fechaFinEventosUsuarios: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoUsuarios').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosUsuarios').val();
            const fechaFinOriginal = $('#fechaFinEventosUsuarios').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de usuarios por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de usuarios por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosUsuariosUserFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoUsuarios').val() !== null && $('#usuarioEventoUsuarios').val().trim() !== '');
