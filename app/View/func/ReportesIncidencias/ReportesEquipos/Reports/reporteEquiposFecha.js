...
$('#reporteEquiposPorFecha').click(function () {
  ...
  if (!validarCamposIncidenciasEquiposFecha()) {
  ...
  $.ajax({
      ...
      try {
        if (data.length > 0) {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const logoWidth = 25;
            const logoHeight = 25;
            const reportTitle = 'REPORTE DE EQUIPOS AFECTADOS POR FECHAS';
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de las incidencias de equipos afectados.', 'Mensaje de error');
...
function validarCamposIncidenciasEquiposFecha() {
  var valido = false;
  var mensajeError = '';
