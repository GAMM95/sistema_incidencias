...
$('#reporteIncidenciasCerradasFecha').click(function () {
  ...
  if (!validarCamposIncidenciasCerradasFecha()) {
  ...
  $.ajax({
    url: 'ajax/ReportesIncidencias/ReportesGenerales/IncidenciasCerradas/getReporteIncidenciasCerradasFecha.php',
    method: 'GET',
    data: {
      ...
      try {
        if (data.length > 0) {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const logoWidth = 25;
            const logoHeight = 25;
            const reportTitle = 'REPORTE DE INCIDENCIAS CERRADAS POR FECHAS';
          ...
          doc.setProperties({
            title: "Reporte de incidencias cerradas por fechas.pdf"
          })
          // Mostrar mensaje de éxito
          toastr.success('Reporte de incidencias cerradas por rango de fechas generado.', 'Mensaje');
...
function validarCamposIncidenciasCerradasFecha() {
  var valido = false;
  var mensajeError = '';
