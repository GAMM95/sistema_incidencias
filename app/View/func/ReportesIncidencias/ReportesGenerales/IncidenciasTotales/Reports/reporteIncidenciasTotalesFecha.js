...
$('#reporteIncidenciasTotalesFecha').click(function () {
  const fechaInicio = $('#fechaInicioIncidenciasTotales').val();
  const fechaFin = $('#fechaFinIncidenciasTotales').val();

  console.log('Fecha Inicio:', fechaInicio);
  console.log('Fecha Fin:', fechaFin);

  // Verificar si los campos son validos
  if (!validarCamposIncidenciasTotalesFecha()) {
  ...
  $.ajax({
    url: 'ajax/ReportesIncidencias/ReportesGenerales/IncidenciasTotales/getReporteIncidenciasTotalesFecha.php',
    method: 'GET',
    data: {
      fechaInicioIncidenciasTotales: fechaInicio, // tiene que estar igual al formulario
      fechaFinIncidenciasTotales: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addDates(doc) {
            ...
            // Obtener valores
            const fechaInicioOriginal = $('#fechaInicioIncidenciasTotales').val();
            const fechaFinOriginal = $('#fechaFinIncidenciasTotales').val();
...
function validarCamposIncidenciasTotalesFecha() {
  var valido = false;
  var mensajeError = '';
