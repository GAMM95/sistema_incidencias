...
WITH IncidenciasOrdenadas AS (
    SELECT
        ...
        B.BIE_nombre,
        I.INC_documento,
        (CONVERT(VARCHAR(10), R.REC_fecha, 103) + ' - ' + STUFF(RIGHT('0' + CONVERT(VARCHAR(7), R.REC_hora, 0), 7), 6, 0, ' ')) AS fechaRecepcionFormateada,
        ...
        CASE
            ...
            ELSE E.EST_descripcion
        END AS Estado,
        -- Agregamos EST_codigo
        CASE
            WHEN C.CIE_numero IS NOT NULL THEN EC.EST_codigo
            ELSE E.EST_codigo
        END AS EST_codigo,
        ROW_NUMBER() OVER (PARTITION BY I.INC_numero ORDER BY I.INC_fecha DESC, I.INC_hora DESC) AS rn
    FROM CIERRE C
    LEFT JOIN MANTENIMIENTO MAN ON MAN.MAN_codigo = C.MAN_codigo
    LEFT JOIN ASIGNACION ASI ON ASI.ASI_codigo = MAN.ASI_codigo
    RIGHT JOIN RECEPCION R ON R.REC_numero = ASI.REC_numero
...
WITH UltimaModificacion AS (
    SELECT 
        ...
        P.PER_nombres + ' ' + P.PER_apellidoPaterno AS usuarioSoporte,
        pA.PER_nombres + ' ' + pA.PER_apellidoPaterno AS usuarioAsignador,
--         ASI.EST_codigo AS estadoAsignacion,
--         C.EST_codigo AS estadoCierre,
--         EM.EST_descripcion,  
        COALESCE(ASI.ASI_fecha, M.MAN_fecha) AS ultimaFecha,
        COALESCE(ASI.ASI_hora, M.MAN_hora) AS ultimaHora,
        ROW_NUMBER() OVER (PARTITION BY ASI.ASI_codigo ORDER BY ISNULL(M.MAN_fecha, ASI.ASI_fecha) DESC, ISNULL(M.MAN_hora, ASI.ASI_hora) DESC) AS rn,
        
        -- Utilizando CASE para determinar el estado final con su código correspondiente
        CASE
            WHEN C.CIE_numero IS NOT NULL THEN EC.EST_codigo  -- Si hay cierre, se toma el código del estado de cierre
            WHEN ASI.EST_codigo = 5 THEN 5 -- Si es estado 5
            WHEN ASI.EST_codigo = 6 THEN 6 -- Si es estado 6
            ELSE NULL -- Por defecto, si no se cumple ninguna de las condiciones
        END AS EST_codigo,

        CASE
            WHEN C.CIE_numero IS NOT NULL THEN EC.EST_descripcion -- Si hay cierre, se toma el estado de cierre
            WHEN ASI.EST_codigo = 5 THEN 'EN PROCESO' -- Si es estado 5
            WHEN ASI.EST_codigo = 6 THEN 'FINALIZADO' -- Si es estado 6
            ELSE 'No asignado' -- Por defecto, si no se cumple ninguna de las condiciones
        END AS Estado
    FROM MANTENIMIENTO M
    RIGHT JOIN ASIGNACION ASI ON ASI.ASI_codigo = M.ASI_codigo
    INNER JOIN RECEPCION R ON R.REC_numero = ASI.REC_numero
    ...
    LEFT JOIN USUARIO U ON U.USU_codigo = ASI.USU_codigo
    INNER JOIN PERSONA P ON P.PER_codigo = U.PER_codigo
    LEFT JOIN ESTADO EM ON EM.EST_codigo = ASI.EST_codigo
    LEFT JOIN CIERRE C ON C.MAN_codigo = M.MAN_codigo
    LEFT JOIN ESTADO EC ON EC.EST_codigo = C.EST_codigo
    WHERE ASI.EST_codigo IN (5,6)
)
SELECT *
...
WITH UltimaModificacion AS (
    SELECT 
        ...
        U.USU_nombre,
        P.PER_nombres + ' ' + P.PER_apellidoPaterno AS usuarioSoporte,
        COALESCE(M.MAN_fecha, R.REC_fecha) AS ultimaFecha,
        COALESCE(M.MAN_hora, R.REC_hora) AS ultimaHora,
        ROW_NUMBER() OVER (PARTITION BY I.INC_numero ORDER BY COALESCE(M.MAN_fecha, R.REC_fecha) DESC, COALESCE(M.MAN_hora, R.REC_hora) DESC) AS rn
    FROM MANTENIMIENTO M
    INNER JOIN ASIGNACION ASI ON ASI.ASI_codigo = M.ASI_codigo
    LEFT JOIN RECEPCION R ON R.REC_numero = ASI.REC_numero
    LEFT JOIN INCIDENCIA I ON I.INC_numero = R.INC_numero
...
SELECT 
    ...
    usuarioSoporte
FROM UltimaModificacion
WHERE rn = 1; -- Solo la última modificación de cada incidencia
GO
...
WITH UltimaModificacion AS (
  SELECT 
    ...
    C.CIE_diagnostico,
    C.CIE_recomendaciones,
    -- Usamos MAX() para las fechas y horas
    ...
    END AS Estado,
    -- Última modificación (fecha y hora más reciente)
    MAX(COALESCE(M.MAN_fecha, C.CIE_fecha)) AS ultimaFecha,
    MAX(COALESCE(M.MAN_hora, C.CIE_hora)) AS ultimaHora,
    ROW_NUMBER() OVER (PARTITION BY M.MAN_codigo ORDER BY MAX(COALESCE(C.CIE_fecha, M.MAN_fecha)) DESC, MAX(COALESCE(C.CIE_hora, M.MAN_hora)) DESC) AS RowNum
  FROM MANTENIMIENTO M
  LEFT JOIN CIERRE C ON C.MAN_codigo = M.MAN_codigo
  RIGHT JOIN ASIGNACION ASI ON ASI.ASI_codigo = M.ASI_codigo
  RIGHT JOIN RECEPCION R ON R.REC_numero = ASI.REC_numero
...
WITH IncidenciasConEquipos AS (
    SELECT
        ...
        I.ultimaFecha,
        I.ultimaHora
    FROM vw_incidencias_totales AS I
    WHERE I.INC_codigoPatrimonial IS NOT NULL
    AND I.INC_codigoPatrimonial <> ''
)
-- Seleccionamos los campos que necesitemos
SELECT 
    INC_numero,
    CIE_numero,
    INC_numero_formato,
...
SELECT
  ...
  U.USU_codigo,
  U.USU_nombre,
  P.PER_nombres + ' ' + P.PER_apellidoPaterno  AS NombreCompleto,
  A.AUD_operacion,
  AR.ARE_nombre,
  A.AUD_ip,
  A.AUD_nombreEquipo 
...
BEGIN
    ...
    DECLARE @HoraSistema TIME(0) = CONVERT(TIME(0), GETDATE()); 
    ...
    BEGIN TRY 
        ...
        ELSE
        BEGIN
         -- Si ya existe una un registro de mantenimiento, solo actualizamos el estado 
...
BEGIN
    ...
    BEGIN TRY 
        ...
        BEGIN
            -- Actualizar el estado del mantenimiento a "inactivo" (estado 2) y borrar la fecha y hora
            UPDATE MANTENIMIENTO
            SET EST_codigo = 2    -- Borrar la hora (poniendo NULL)
            WHERE ASI_codigo = @ASI_codigo;
            
            -- Actualizar el estado de la asignación a "EN ESPERA" (estado 5)
            UPDATE ASIGNACION 
            SET EST_codigo = 5  -- Estado "EN ESPERA"
...
BEGIN
  ...
  WHERE (@codigoPatrimonial IS NULL OR INC_codigoPatrimonial = @codigoPatrimonial)
    AND (@fechaInicio IS NULL OR INC_fecha >= @fechaInicio)
...
BEGIN
    SELECT 
        ...
        fechaMantenimientoFormateada,
        tiempoMantenimientoSegundos,
        tiempoMantenimientoFormateado,
        MAN_fecha,
        ARE_nombre,
        INC_asunto,
        INC_documento,
        ...
        ultimaFecha,
        ultimaHora
    FROM vw_incidencias_mantenimiento
    WHERE
        (@usuario IS NULL OR USU_codigo = @usuario) AND
        (@fechaInicio IS NULL OR ultimaFecha >= @fechaInicio) AND
        (@fechaFin IS NULL OR ultimaFecha <= @fechaFin)
    ORDER BY ultimaFecha DESC, ultimaHora DESC;
END;
GO
...
BEGIN
  ...
  WHERE 
    Estado = 'CERRADO'
    AND (@usuario IS NULL OR USU_codigo = @usuario)
    AND (@fechaInicio IS NULL OR ultimaFecha >= @fechaInicio)
    AND (@fechaFin IS NULL OR ultimaFecha <= @fechaFin)
  ORDER BY ultimaFecha DESC, ultimaHora DESC;
END
...
BEGIN
  SELECT 
    ...
    ultimaFecha,
    ultimaHora
  FROM vw_reporte_incidencias_area
  WHERE 
    (@area IS NULL OR ARE_codigo = @area) AND
    (@fechaInicio IS NULL OR INC_fecha >= @fechaInicio) AND
    (@fechaFin IS NULL OR INC_fecha <= @fechaFin) 
  ORDER BY 
...
BEGIN
  SELECT 
    ...
    Estado,
    USU_nombre AS Usuario
  FROM vw_reporte_incidencias_equipos
  WHERE 
    (@fechaInicio IS NULL OR INC_fecha >= @fechaInicio) AND
    (@fechaFin IS NULL OR INC_fecha <= @fechaFin) AND
    (@codigoPatrimonial IS NULL OR INC_codigoPatrimonial = @codigoPatrimonial)
  ORDER BY 
    SUBSTRING(INC_numero_formato, CHARINDEX('-', INC_numero_formato) + 1, 4) DESC,
    INC_numero_formato DESC;
END
...
BEGIN
  ...
  WHERE 
    (@codigoPatrimonial IS NULL OR codigoPatrimonial = @codigoPatrimonial)
    AND (@fechaInicio IS NULL OR codigoPatrimonial IN (
        SELECT INC_codigoPatrimonial
        FROM INCIDENCIA
        WHERE INC_fecha >= @fechaInicio
    ))
    AND (@fechaFin IS NULL OR codigoPatrimonial IN (
        SELECT INC_codigoPatrimonial
        FROM INCIDENCIA
        WHERE INC_fecha <= @fechaFin
    ))
  ORDER BY cantidadIncidencias DESC;
END
GO
...
-- PROCEDIMIENTO ALMACENADO PARA CONSULTAR EVENTOS DE MANTENIMIENTO
CREATE OR ALTER PROCEDURE sp_consultar_eventos_mantenimiento
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
...
-- PROCEDIMIENTO ALMACENADO PARA CONSULTAR EVENTOS DE CIERRES
CREATE OR ALTER PROCEDURE sp_consultar_eventos_cierres
  @usuario INT = NULL,
  @fechaInicio DATE = NULL,
  @fechaFin DATE = NULL
AS
