...
$('#reporteEventosAsignacionesUsuario').click(function () {
  const usuario = $("#usuarioEventoAsignaciones").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosAsignacionesUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Asignaciones/getReporteAuditoriaAsignacionesUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de asignaciones por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de asignaciones por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de asignaciones para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de asignaciones.', 'Mensaje de error');
...
function validarCamposEventosAsignacionesUsuario() {
  var valido = false;
  var mensajeError = '';
