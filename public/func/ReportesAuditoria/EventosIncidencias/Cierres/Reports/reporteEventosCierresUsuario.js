...
$('#reporteEventosCierresUsuario').click(function () {
  const usuario = $("#usuarioEventoCierres").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosCierresUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Cierres/getReporteAuditoriaCierresUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de cierres por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de cierres por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de cierres para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de cierres.', 'Mensaje de error');
...
function validarCamposEventosCierresUsuario() {
  var valido = false;
  var mensajeError = '';
