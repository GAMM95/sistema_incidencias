...
$('#reporteEventosMantenimientoUsuario').click(function () {
  const usuario = $("#usuarioEventoMantenimiento").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosMantenimientoUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Mantenimiento/getReporteAuditoriaMantenimientoUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de mantenimiento por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de mantenimiento por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de mantenimiento para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de mantenimiento.', 'Mensaje de error');
...
function validarCamposEventosMantenimientoUsuario() {
  var valido = false;
  var mensajeError = '';
