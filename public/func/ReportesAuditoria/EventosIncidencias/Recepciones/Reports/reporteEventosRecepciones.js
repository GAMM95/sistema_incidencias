$(document).ready(function () {
  ...
  $('#reporteEventosTotales').click(function () {
    // Realizar la solicitud AJAX para obtener los datos
    $.ajax({
      url: 'ajax/ReportesAuditoria/EventosIncidencias/Recepciones/getReporteAuditoriaRecepcionesTotales.php',
      method: 'GET',
      dataType: 'json',
      success: function (data) {
        ...
        try {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const reportTitle = 'REPORTE DE EVENTOS DE RECEPCIONES';
          ...
          doc.setProperties({
            title: 'Reporte total de eventos de recepciones.pdf',
          });
          // Mostrar mensaje de exito de pdf generado
          toastr.success('Reporte total de eventos de recepciones generado.', 'Mensaje');
          // Retrasar la apertura del PDF y limpiar el campo de entrada
          setTimeout(() => {
            window.open(doc.output('bloburl'), '_blank');
          }, 2000);
        } catch (error) {
          toastr.error('Hubo un error al generar el reporte total eventos de recepciones.', 'Mensaje de error');
          console.error('Error al generar el PDF:', error.message);
        }
      },
      error: function (xhr, status, error) {
        toastr.error('Hubo un error al obtener el reporte total de eventos de recepciones.', 'Mensaje de error');
