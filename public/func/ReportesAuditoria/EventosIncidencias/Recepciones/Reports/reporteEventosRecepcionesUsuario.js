...
$('#reporteEventosRecepcionesUsuario').click(function () {
  const usuario = $("#usuarioEventoRecepciones").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosRecepcionesUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosIncidencias/Recepciones/getReporteAuditoriaRecepcionesUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de recepciones por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de recepciones por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de recepciones para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
...
function validarCamposEventosRecepcionesUsuario() {
  var valido = false;
  var mensajeError = '';
