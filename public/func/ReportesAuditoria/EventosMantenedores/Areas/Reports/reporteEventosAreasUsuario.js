...
$('#reporteEventosAreasUsuario').click(function () {
  const usuario = $("#usuarioEventoAreas").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosAreasUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Areas/getReporteAuditoriaAreasUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de áreas por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de &aacute;reas por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de &aacute;reas para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de &aacute;reas.', 'Mensaje de error');
...
function validarCamposEventosAreasUsuario() {
  var valido = false;
  var mensajeError = '';
