...
$('#reporteEventosAreasUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoAreas").val();
  const fechaInicio = $('#fechaInicioEventosAreas').val();
  const fechaFin = $('#fechaFinEventosAreas').val();
  ...
  if (!validarCamposEventosAreasUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Areas/getReporteAuditoriaAreasUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoAreas: usuario,
      fechaInicioEventosAreas: fechaInicio,
      fechaFinEventosAreas: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoAreas').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosAreas').val();
            const fechaFinOriginal = $('#fechaFinEventosAreas').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de áreas por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de &aacute;reas por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosAreasUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoAreas').val() !== null && $('#usuarioEventoAreas').val().trim() !== '');
