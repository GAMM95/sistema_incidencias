...
$('#reporteEventosBienesUsuario').click(function () {
  const usuario = $("#usuarioEventoBienes").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosBienesUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Bienes/getReporteAuditoriaBienesUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de bienes por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de bienes por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de bienes para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de bienes.', 'Mensaje de error');
...
function validarCamposEventosBienesUsuario() {
  var valido = false;
  var mensajeError = '';
