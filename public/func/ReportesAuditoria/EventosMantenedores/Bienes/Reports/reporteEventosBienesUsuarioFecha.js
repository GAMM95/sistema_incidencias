...
$('#reporteEventosBienesUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoBienes").val();
  const fechaInicio = $('#fechaInicioEventosBienes').val();
  const fechaFin = $('#fechaFinEventosBienes').val();
  ...
  if (!validarCamposEventosBienesUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Bienes/getReporteAuditoriaBienesUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoBienes: usuario,
      fechaInicioEventosBienes: fechaInicio,
      fechaFinEventosBienes: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoBienes').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosBienes').val();
            const fechaFinOriginal = $('#fechaFinEventosBienes').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de bienes por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de bienes por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosBienesUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoBienes').val() !== null && $('#usuarioEventoBienes').val().trim() !== '');
