...
$('#reporteEventosPersonasFecha').click(function () {
  ...
  if (!validarCamposEventosPersonasFecha()) {
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Personas/getReporteAuditoriaPersonasFecha.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de personas por rango de fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de personas por rango de fechas generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de personas por rango de fechas generado.', 'Advertencia');
        }
      } catch (error) {
...
function validarCamposEventosPersonasFecha() {
  var valido = false;
  var mensajeError = '';
