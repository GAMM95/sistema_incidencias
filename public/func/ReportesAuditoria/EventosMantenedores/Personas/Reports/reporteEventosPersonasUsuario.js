...
$('#reporteEventosPersonasUsuario').click(function () {
  const usuario = $("#usuarioEventoPersonas").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposEventosPersonasUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Personas/getReporteAuditoriaPersonasUsuario.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de personas por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de personas por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de personas para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de personas.', 'Mensaje de error');
...
function validarCamposEventosPersonasUsuario() {
  var valido = false;
  var mensajeError = '';
