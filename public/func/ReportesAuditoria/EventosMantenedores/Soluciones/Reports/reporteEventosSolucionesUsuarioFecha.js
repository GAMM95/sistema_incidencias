...
$('#reporteEventosSolucionesUsuarioFecha').click(function () {
  const usuario = $("#usuarioEventoSoluciones").val();
  const fechaInicio = $('#fechaInicioEventosSoluciones').val();
  const fechaFin = $('#fechaFinEventosSoluciones').val();
  ...
  if (!validarCamposEventosSolucionesUsuarioFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Soluciones/getReporteAuditoriaSolucionesUsuarioFecha.php',
    method: 'GET',
    data: {
      usuarioEventoSoluciones: usuario,
      fechaInicioEventosSoluciones: fechaInicio,
      fechaFinEventosSoluciones: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addUserAndDates(doc) {
            ...
            // Obtener valores
            const usuarioNombre = $('#nombreUsuarioEventoSoluciones').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioEventosSoluciones').val();
            const fechaFinOriginal = $('#fechaFinEventosSoluciones').val();
          ...
          doc.setProperties({
            title: 'Reporte de eventos de soluciones por usuario y fechas.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de soluciones por usuario y fechas generado.', 'Mensaje');
...
function validarCamposEventosSolucionesUsuarioFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var usuarioSeleccionado = ($('#usuarioEventoSoluciones').val() !== null && $('#usuarioEventoSoluciones').val().trim() !== '');
