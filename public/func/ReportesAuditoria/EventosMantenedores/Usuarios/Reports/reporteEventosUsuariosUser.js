...
$('#reporteUserEventosUsuario').click(function () {
  ...
  $.ajax({
    url: 'ajax/ReportesAuditoria/EventosMantenedores/Usuarios/getReporteAuditoriaUsuariosUser.php',
    method: 'GET',
    data: {
    ...
    success: function (data) {
      ...
      try {
        if (data.length > 0) {
          ...
          doc.setProperties({
            title: 'Reporte de eventos de usuarios por usuario.pdf'
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de eventos de usuarios por usuario generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado eventos de usuarios para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de los eventos de usuarios.', 'Mensaje de error');
...
function validarCamposEventosUsuariosUser() {
  var valido = false;
  var mensajeError = '';
