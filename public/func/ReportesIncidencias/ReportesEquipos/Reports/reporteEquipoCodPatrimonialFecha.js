...
$('#reporteEquiposPorCodPatrimonialFecha').click(function () {
  const equipo = $("#codigoPatrimonialEquipo").val();
  const fechaInicio = $('#fechaInicioIncidenciasEquipo').val();
  const fechaFin = $('#fechaFinIncidenciasEquipo').val();

  console.log('Código Patrimonial:', equipo);
  console.log('Fecha Inicio:', fechaInicio);
  console.log('Fecha Fin:', fechaFin);

  // Verificar si los campos son validos
  if (!validarCamposEquipoCodigoPatrimonialFecha()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesIncidencias/ReporteEquipos/getReportePorCodigoPatrimonialFecha.php',
    method: 'GET',
    data: {
      codigoPatrimonialEquipo: equipo,
      fechaInicioIncidenciasEquipo: fechaInicio,
      fechaFinIncidenciasEquipo: fechaFin
    },
    dataType: 'json',
      ...
      try {
        if (data.length > 0) {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const logoWidth = 25;
            const logoHeight = 25;
            const reportTitle = 'REPORTE DE INCIDENCIAS DE EQUIPOS';
          ...
          function addEquipoAndDates(doc) {
            ...
            // Obtener valores
            const equipoNombre = $('#codigoPatrimonialEquipo').val() || '-';
            const fechaInicioOriginal = $('#fechaInicioIncidenciasEquipo').val();
            const fechaFinOriginal = $('#fechaFinIncidenciasEquipo').val();
          ...
          doc.setProperties({
            title: "Reporte de equipos afectados por código patrimonial y fechas.pdf"
          });

          // Mostrar mensaje de éxito
          toastr.success('Reporte de incidencias de equipos por c&oacute;digo patrimonial y fechas generado.', 'Mensaje');
          ...
          toastr.warning('No se ha encontrado incidencias equipos para los campos ingresados.', 'Advertencia');
        }
      } catch (error) {
    ...
    error: function (xhr, status, error) {
      toastr.error('Hubo un error al obtener datos de las incidencias equipos.', 'Mensaje de error');
      console.error('Error al realizar la solicitud AJAX:', error);
    }
  });
});

function validarCamposEquipoCodigoPatrimonialFecha() {
  var valido = false;
  var mensajeError = '';
  ...
  var equipo = ($('#codigoPatrimonialEquipo').val() !== null && $('#codigoPatrimonialEquipo').val().trim() !== '');

  // Verificar si al menos uno de los campos tiene datos
  if (fechaInicioSeleccionada && fechaFinSeleccionada && equipo) {
    valido = true;
  } else {
    mensajeError = 'Debe ingresar el c&oacute;digo patrimonial y el rango de fechas para generar el reporte.';
  }
