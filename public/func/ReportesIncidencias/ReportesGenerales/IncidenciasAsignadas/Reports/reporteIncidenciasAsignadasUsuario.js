...
$('#reporteIncidenciasAsignadasUsuario').click(function () {
  const usuario = $("#usuarioIncidenciasAsignadas").val();

  console.log('Usuario:', usuario);

  // Verificar si los campos son validos
  if (!validarCamposIncidenciasAsignadasUsuario()) {
    return;
  }
  ...
  $.ajax({
    url: 'ajax/ReportesIncidencias/ReportesGenerales/IncidenciasAsignadas/getReporteIncidenciasAsignadasUsuario.php',
    method: 'GET',
    data: {
      ...
      try {
        if (data.length > 0) {
          ...
          function addHeader(doc, totalRecords) {
            ...
            const logoWidth = 25;
            const logoHeight = 25;
            const reportTitle = 'REPORTE DE INCIDENCIAS ASIGNADAS POR USUARIO';
          ...
          doc.setProperties({
            title: "Reporte de incidencias asignadas por usuario.pdf"
          })
          // Mostrar mensaje de éxito
          toastr.success('Reporte de incidencias asignadas por usuario generado.', 'Mensaje');
          // Abrir PDF después de una pequeña pausa
          setTimeout(() => {
            window.open(doc.output('bloburl'),'_blank');
          }, 2000);
        } else {
          toastr.warning('No se ha encontrado incidencias asignadas para el usuario seleccionado.', 'Advertencia');
        }
      } catch (error) {
...
function validarCamposIncidenciasAsignadasUsuario() {
  var valido = false;
  var mensajeError = '';
